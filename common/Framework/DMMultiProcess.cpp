...
DMMultiProcess::DMMultiProcess():_process_num(0)
{
...
DM_INT DMMultiProcess::set_process_options(ACE_Process_Options& options)
{
    if (_argc == 1) //parent
    {
        _process_num = DMJsonCfg::instance()->GetItemInt("service_config", "process_num");
...
DM_INT DMMultiProcess::create_process(DM_INT argc, DM_CHAR *argv[])
{     
    _argc = argc;
    _argv = argv;
